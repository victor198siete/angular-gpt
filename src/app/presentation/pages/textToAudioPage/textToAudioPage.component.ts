...
export default class TextToAudioPageComponent {
  ...
  handleMessageWithSelect( {prompt, selectedOption}: TextMessageBoxEvent ) {
    ...
    this.messages.update( prevMessages => [...prevMessages, {text: message, isGpt: false}]);
    this.isLoading.set(true);

    this.openAiService.textToAudio(prompt, selectedOption)
    .subscribe( ({ message, audioUrl }) => {
      this.isLoading.set(false);
      this.messages.update( prevMessages => [...prevMessages, {text: message, isGpt: true, audioUrl: audioUrl}]);
    })
